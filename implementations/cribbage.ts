...
export class Cribbage extends CardGame<CribbagePlayer, StandardDeck> {
    ...
    if (this.mode === Mode.Team) {
      if (this.numPlayers === 4) {
        this.teams = new Teams(
          new ItemCollection<CribbageTeam>([
            new CribbageTeam(1, [
              this.players.itemAt(0),
              this.players.itemAt(2),
            ]),
            new CribbageTeam(2, [
              this.players.itemAt(1),
              this.players.itemAt(3),
            ]),
          ])
        );
      } else {
        this.teams = new Teams(
          new ItemCollection<CribbageTeam>([
            ...
            new CribbageTeam(2, [
              this.players.itemAt(1),
              this.players.itemAt(4),
            ]),
            new CribbageTeam(3, [
              this.players.itemAt(2),
              this.players.itemAt(5),
            ]),
          ])
        );
      }
      ...
      if (this.teams !== null) {
        ...
        this.teams = new Teams<CribbagePlayer>(
          new ItemCollection<CribbageTeam>([])
        );
      }
      let id = 1;
  ...
  private initializeGame(): void {
    ...
    this.mode =
      this.numPlayers === 4 || this.numPlayers === 6 ? Mode.Team : Mode.FFA;
  ...
  giveToKitty(playerName: string, cards: ItemCollection<Card>): CribbageReturn {
    ...
    for (let ix = 0; ix < numThrown; ix++) {
      player.hand.playCard(
        player.hand.itemAt(player.hand.indexOfItem(cards.itemAt(ix)))
      );
    }
    ...
    if (this.kitty.size() === 4) {
      ...
      if (this.cut.value === Value.Jack) {
        ...
        if (dealingTeam.addPoints(this.dealer, 2)) {
          ...
          response.message = `2 points to ${
            this.dealer.name
          } ${Cribbage.teamPointsString(
            dealingTeam
          )} for 'His Heels' (cutting the right jack)`;
        }
      }
    }
    return response;
  ...
  playCard(playerName: string, card: Card): CribbageReturn {
    ...
    let points: number, team: CribbageTeam;
    while (true) {
      team = this.teams.findTeam(player) as CribbageTeam;
      ...
      const is31 = this.count === 31,
        is15 = this.count === 15;
      if (is15 || is31) {
        response.message = `${player.name} gets ${
          is15 ? 15 : 31
        } for two points`;
        ...
        points += 2;
        if (prevPoints > 0) {
          response.message += ` in addition to ${prevPoints} points from the run-of-play ${Cribbage.teamPointsString(
            team
          )}.\n`;
        } else {
      ...
      if (this.roundOver()) {
        ...
        if (points > 0) {
          response.message += `${
            player.name
          } scored ${points} points ${Cribbage.teamPointsString(team)}.`;
        }
        response.message += `\nThe sequence has been reset, the count is at ${this.count}.`;
        break;
      } else {
        this.nextPlayerInSequence = this.nextPlayerInOrder(
          this.nextPlayerInSequence
        );
        this.setNextPlayerInSequence(player);
        break;
      }
  ...
  go(playerName: string): CribbageReturn {
    ...
    if (this.playersInPlay.countItems() === 0) {
      ...
      if (team.addPoints(this.lastPlayerToPlay, 1)) {
        ...
        response.message += ` ${Cribbage.teamPointsString(
          team
  ...
  private setGameOver(winningTeam: CribbageTeam): CribbageReturn {
    ...
    for (let ix = 0; ix < this.teams.numTeams(); ix++) {
      ...
      if (!team.equalsOther(winningTeam)) {
        otherScores += `Losing team: ${team.printTeam()} ${Cribbage.teamPointsString(
          team
        )} ${Cribbage.loserEmoji}\n`;
      }
    }
    return new CribbageReturn(
      ...
      }\nWinning team: ${this.winningTeam.printTeam()} ${Cribbage.teamPointsString(
        winningTeam
      )} ${Cribbage.winnerEmoji}\n${otherScores}`
    );
  }
  ...
  private countPoints(): CribbageReturn {
    let ret = new CribbageReturn(
      false,
      `${Cribbage.cutEmoji}  The cut card is the ${this.cut}`
    );
    ...
    do {
      ...
      ret.message += `\n${countingPlayer.name} has hand ${Cribbage.printHand(
        <CribbageHand>countingPlayer.hand
      )} and scored ${points} points ${Cribbage.teamPointsString(team)}.`;
      if (this.dealer.equalsOther(countingPlayer)) {
        ...
        ret.message += `\nThe kitty is ${Cribbage.printHand(
          this.kitty
        )} and scores ${points} points for ${
          countingPlayer.name
        } ${Cribbage.teamPointsString(team)}.`;
      }
  ...
  private setNextPlayerInSequence(player: CribbagePlayer): void {
    if (player !== null) {
      if (
        this.nextPlayerInSequence.equalsOther(player) ||
        this.playersInPlay.indexOfItem(this.nextPlayerInSequence) === -1
      ) {
  ...
  private dealForSix(): void {
    ...
    while (player.numCards() < 5) {
      if (
        !(
          player.equalsOther(this.dealer) ||
          dealingTeam.equalsOther(this.findTeam(player))
        )
      ) {
        player.takeCard(this.draw());
      }
  ...
  private findTeam(player: CribbagePlayer): CribbageTeam {
    let team: CribbageTeam;
    for (let index = 0; index < this.teams.numTeams(); index++) {
      const t = this.getTeam(index) as CribbageTeam;
